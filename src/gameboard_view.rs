...
pub struct GameboardViewSettings {
    ...
    /// Error cell Background color
    pub error_cell_backround_color: Color,
}
...
impl GameboardViewSettings {
    ...
    pub fn new() -> GameboardViewSettings {
        GameboardViewSettings {
            ...
            error_cell_backround_color: [1.0, 0.0, 0.0, 0.8],
        }
    }
}
...
impl GameboardView {
    ...
    /// Draw gameboard.
    pub fn draw<G: Graphics, C>(&self, controller: &GameboardController,
                                glyphs: &mut C,
                                c: &Context, g: &mut G)
        ...
        if let Some(ind) = controller.selected_cell {
            ...
            let cell_color= match controller.gameboard.error_cell {
                Some(error_ind) => {
                    match error_ind == ind {
                        true => {
                            println!("Error cell at {:?}", ind);
                            settings.error_cell_backround_color
                        },
                        false => settings.selected_cell_background_color
                    }
                },
                None => settings.selected_cell_background_color
            };
